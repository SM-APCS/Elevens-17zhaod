...
public class ElevensBoard extends Board {
	...
	private boolean playPairSum11IfPossible() {
	  ...
	  {
	    if(cardAt(i) != null){x.add(i);}
      //else {x.add(-1);}
	    for(int j=i+1;j<BOARD_SIZE;j++)
	    {
	      if(cardAt(j) != null){x.add(j);}
        //else {x.add(-1);}
	      if(findPairSum11(x))
	      {
	        replaceSelectedCards(x);
	        return true;
	      }
	      if((x.size() != 0) && (cardAt(j) != null)) {x.remove(x.size()-1);}
	    }
	    if(x.size() != 0) {x.remove(x.size()-1);}
	  }
	    ...
	    for(int i=0;i<BOARD_SIZE-2;i++)
	    {
	      ...
	      for(int j=i+1;j<BOARD_SIZE-1;j++)
	      {
	        ...
	        for(int k=j+1;k<BOARD_SIZE;k++)
	        {
  	        ...
  	        if((x.size() != 0) && (cardAt(k) != null)) {x.remove(x.size()-1);}
	        }
	        if((x.size() != 0) && (cardAt(j) != null)) {x.remove(x.size()-1);}
	      }
	      if(x.size() != 0) {x.remove(x.size()-1);}
